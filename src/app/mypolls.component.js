"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var poll_service_1 = require("./poll.service");
var MyPollsComponent = (function () {
    function MyPollsComponent(pollService) {
        this.pollService = pollService;
        this.data = [];
        this.warning = false;
        this.errMsg = '';
    }
    MyPollsComponent.prototype.getMyPolls = function () {
        var _this = this;
        this.pollService.getMyPolls().then(function (res) {
            if (res.length > 0) {
                _this.warning = false;
                _this.myPolls = res;
            }
            else {
                _this.warning = true;
            }
        }, function (err) { return _this.errMsg = err; });
    };
    MyPollsComponent.prototype.ngOnInit = function () {
        this.getMyPolls();
    };
    MyPollsComponent.prototype.delete = function (poll) {
        var _this = this;
        this.pollService.delete(poll.id)
            .then(function () { return _this.myPolls = _this.myPolls.filter(function (p) { return p !== poll; }); }, function (err) { return _this.errMsg = err; });
    };
    ;
    return MyPollsComponent;
}());
MyPollsComponent = __decorate([
    core_1.Component({
        templateUrl: './mypolls.component.html',
        styleUrls: ['./poll-list.component.css', './mypolls.component.css']
    }),
    __metadata("design:paramtypes", [poll_service_1.PollService])
], MyPollsComponent);
exports.MyPollsComponent = MyPollsComponent;
//# sourceMappingURL=mypolls.component.js.map